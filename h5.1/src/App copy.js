 ...
 * Card
    ...
    During click events, check Card.reveal. This can check a lot of logic for you. For example...
    If Card.reveal === true, either the card has just been selected, or it was already set to true after the player answered correctly. If Card.reveal is true, simply return. 
    A good place to set Card.reveal is when checking the answer.
 */
  ...
  return (
    <div className="app">
      <div className="deck-container">
        { randomCards.map( 
          (val, i) => <Card cardHandler={cardHandler} id={"card_"+i} key={i} pairId={val.pairId} word={(val.hasOwnProperty("en") ? val.en : val.fi)} show={show} /> 
        )}
      </div>
    </div>
  );
